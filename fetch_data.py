...
def mkdir_dataset():
    ...
    datasets_folder = os.path.abspath(os.path.join(curr, 'datasets'))
...
def download_pack(datasets_folder):
    ...
    print ("Checking that the Caret file exists...", file_path)
    assert os.path.exists(file_path)
    print ("=> Success!")
    os.remove(archive_path)
    ...
    for root, dirs, files in os.walk(datasets_folder):
        for name in files:
            ...
            if name.endswith('.RData'):
                ...
                # r.data(robj)
                for var in robj:
                    myRData = pandas2ri.ri2py(r[var])

                    if isinstance(myRData,  np.ndarray):
                        myRData = pd.DataFrame(myRData)

                    if isinstance(myRData, pd.core.categorical.Categorical):
                        myRData = pd.DataFrame(myRData.get_values())
